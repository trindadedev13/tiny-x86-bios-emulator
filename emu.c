...
void
txbe_emu_handle_interrupt (uint8_t int_num)
{
  if (int_num == 0x10)
    {
      txbe_int10h_handle ();
...
int
txbe_emu_emulate_next (uint16_t physical_ip)
{
  uint8_t opcode = ram[physical_ip];

  uint16_t *reg16[] = { &cpu.ax, &cpu.cx, &cpu.dx, &cpu.bx,
                        &cpu.sp, &cpu.bp, &cpu.si, &cpu.di };
  uint8_t *reg8_low[] = { (uint8_t *)&cpu.ax, (uint8_t *)&cpu.cx,
                          (uint8_t *)&cpu.dx, (uint8_t *)&cpu.bx };
  uint8_t *reg8_high[]
      = { ((uint8_t *)&cpu.ax) + 1, ((uint8_t *)&cpu.cx) + 1,
          ((uint8_t *)&cpu.dx) + 1, ((uint8_t *)&cpu.bx) + 1 };
